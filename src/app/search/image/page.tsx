...
export default async function ImageSearchPage({ searchParams }: Params) {
  ...
  https://www.googleapis.com/customsearch/v1?key=${process.env.GOOGLE_SEARCH_ENGINE_API_KEY}&cx=${process.env.GOOGLE_SEARCH_ENGINE_CONTEXT_KEY}&q=${searchParams.searchTerm}&searchType=image&start=${startIndex}
  `);
  if (!response.ok) {
